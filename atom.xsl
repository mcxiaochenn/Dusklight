...
<div class="container">
  <div class="hero">
    ...
    <h1 class="hero__title"><xsl:value-of select="atom:feed/atom:title" /></h1>
    <p class="hero__desc"><xsl:value-of select="atom:feed/atom:subtitle" /></p>
    <span class="hero__badge">Atom 1.0 订阅源</span>
  </div>
  <div class="list">
    <xsl:for-each select="atom:feed/atom:entry">
    <article class="item">
      <h2 class="item__title"><a href="{atom:link/@href}"><xsl:value-of select="atom:title" /></a></h2>
      <time class="item__date"><xsl:value-of select="atom:published" /></time>
      <p class="item__desc"><xsl:value-of select="atom:summary" /></p>
    </article>
    </xsl:for-each>
  </div>
  <footer class="footer">
    <xsl:value-of select="atom:feed/atom:title" /><xsl:value-of select="' '" />
